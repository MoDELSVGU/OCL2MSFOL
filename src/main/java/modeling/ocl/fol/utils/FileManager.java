...
package modeling.ocl.fol.utils;
...
public final class FileManager {
	...
	public void init() throws IOException {
		...
		if (!safeMode)
			writeln("(declare-sort Type 0)");
		writeln("(declare-const nullClassifier Classifier)");
		if (!safeMode)
			writeln("(declare-const invalidClassifier Classifier)");
		writeln("(declare-const nullInt Int)");
		if (!safeMode)
			writeln("(declare-const invalidInt Int)");
		writeln("(declare-const nullString String)");
		if (!safeMode)
			writeln("(declare-const invalidString String)");
		if (!safeMode)
			assertln("(distinct nullClassifier invalidClassifier)");
		if (!safeMode)
			assertln("(distinct nullInt invalidInt)");
		if (!safeMode)
			assertln("(distinct nullString invalidString)");
	}
